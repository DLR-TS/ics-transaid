...
#include "ns3/simulator.h"
#include <map> // added by J. Haerri for STF447 first
...
void
Node::AddChannelLoadMonitor (Ptr<ChannelLoadMonitorMngr> channelLoadMngr)
{
	for (std::vector<Ptr<NetDevice> >::iterator i = m_devices.begin ();
	               i != m_devices.end ()-1 ; i++) // there is -1 just because we use only CCh and Sch1
	   {
		Ptr<NetDevice> dev = *i;
		Ptr<ChannelLoadMonitorMngr> CLmngr =  dev->GetNode()->GetObject<ChannelLoadMonitorMngr>();
		CLmngr=channelLoadMngr;
		CLmngr->SetNetDevice(dev);
		m_ChannelLoadMonitorMngr.push_back(CLmngr);
	   }
}

void
Node::AddChannelLoadMonitor (uint32_t NetDevice_id, Ptr<ChannelLoadMonitorMngr> channelLoadMngr)
{

	for (std::vector<Ptr<NetDevice> >::iterator i = m_devices.begin ();
		               i != m_devices.end (); i++){
		Ptr<NetDevice> dev = *i;
		if (dev->GetIfIndex() == NetDevice_id){
			channelLoadMngr->SetNetDevice(dev);
			m_ChannelLoadMonitorMngr.push_back(channelLoadMngr);
		}
	}
}

void
Node::StartChannelLoadMonitoring ()
{
	for (std::vector<Ptr<ChannelLoadMonitorMngr> >::iterator i = m_ChannelLoadMonitorMngr.begin();
		       i != m_ChannelLoadMonitorMngr.end (); i++)
		 {
		   Ptr<ChannelLoadMonitorMngr> channelLoadMonitorMngr = *i;
		   channelLoadMonitorMngr->startMonitoring();
	}
}

void
Node::StopChannelLoadMonitoring ()
{
	for (std::vector<Ptr<ChannelLoadMonitorMngr> >::iterator i = m_ChannelLoadMonitorMngr.begin ();
		       i != m_ChannelLoadMonitorMngr.end (); i++)
		 {
		   Ptr<ChannelLoadMonitorMngr> channelLoadMonitorMngr = *i;
		   channelLoadMonitorMngr->stopMonitoring();
	}
}

float
Node::GetChannelLoad (uint32_t NetDevice_id)
{
	  for (std::vector<Ptr<ChannelLoadMonitorMngr> >::iterator i = m_ChannelLoadMonitorMngr.begin ();
	       i != m_ChannelLoadMonitorMngr.end (); i++)
	    {
		  Ptr<ChannelLoadMonitorMngr> channelLoadMonitorMngr = *i;
		  if (((channelLoadMonitorMngr->GetNetDevice())->GetIfIndex()) == NetDevice_id)
		  {
			  return channelLoadMonitorMngr->GetChannelLoad();
		  }
	    }
	  return -1.0;
}
...
Node::GetChannelLoad ()
{
	std::vector<float> load_vector;
	 ...
	 for (std::vector<Ptr<ChannelLoadMonitorMngr> >::iterator i = m_ChannelLoadMonitorMngr.begin ();
		 ...
		 {
			  Ptr<ChannelLoadMonitorMngr> channelLoadMonitorMngr = *i;
			  load_vector.push_back(channelLoadMonitorMngr->GetChannelLoad());
		 }
...
std::string
Node::GetChannelState (uint32_t NetDevice_id)
{
	for (std::vector<Ptr<ChannelLoadMonitorMngr> >::iterator i = m_ChannelLoadMonitorMngr.begin ();
		       i != m_ChannelLoadMonitorMngr.end (); i++)
		    {
			  ...
			  if (((channelLoadMonitorMngr->GetNetDevice())->GetIfIndex()) == NetDevice_id)
			  {
				  return channelLoadMonitorMngr->GetChannelState();
			  }
		    }
		  return state4;
}
...
{
	std::vector<std::string> channelStage_vector;

	for (std::vector<Ptr<ChannelLoadMonitorMngr> >::iterator i = m_ChannelLoadMonitorMngr.begin ();
		       i != m_ChannelLoadMonitorMngr.end (); i++)
		 {
			  Ptr<ChannelLoadMonitorMngr> channelLoadMonitorMngr = *i;
			  channelStage_vector.push_back(channelLoadMonitorMngr->GetChannelState());
		 }
	return channelStage_vector;
}
