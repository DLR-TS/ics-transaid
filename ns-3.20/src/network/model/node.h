...
#include "ns3/channel-load-monitor-mngr.h"  // added by J. Haerri for STF447
#include <map>						 	   // added by J. Haerri for STF447
...
public:
  ...
  */
  ...
  bool IsMobileNode (void); // Added by Ramon Bauza first, then by Andrea
  void SetMobileNode (bool mobileNode); // Added by Ramon Bauza first then by Andrea

  // TODO: JHNote: Should be moved to a different class...for example, vehicle-sta-mgnt or rsu-sta-mgnt, or directly on the channel load class, accessed by aggregation

  /**
   * @brief Add a Channel Load Monitor Manager to the given NetDevice. No check is performed on the type of ChannelLoadMonitor
   * Note: method used to add a specific ChannelLoadMonitor per netdevice (when testing difference channel load monitoring algorithms)
   * if all NetDevice shall have a given ChannelLoadMonitor, better use AddChannelLoadMonitor(Ptr<ChannelLoadMonitorMngr>)
   *
   */
  void AddChannelLoadMonitor (uint32_t NetDevice_id, Ptr<ChannelLoadMonitorMngr> channelLoadMngr); // added by J. Haerri (EURECOM)

  /**
  * @brief Add a Channel Load Monitor Manager to the given NetDevice. No check is performed on the type of ChannelLoadMonitor
  * Note: method used to add a specific ChannelLoadMonitor to all NetDevice
   *
  */
  void AddChannelLoadMonitor (Ptr<ChannelLoadMonitorMngr> channelLoadMngr); // added by J. Haerri (EURECOM)

  /**
  * @brief Returns the Channel Load of the NetDevice NetDevice_id. If the NetDevice is not found, the function returns a negative value (-1.0)
  */
  float GetChannelLoad (uint32_t NetDevice_id); // added by J. Haerri (EURECOM)

  /**
  * @brief Returns the Channel Load of all NetDevices associated to this node. If none is found, vector will be empty
  */
  std::vector<float> GetChannelLoad (); // added by J. Haerri (EURECOM)

  /**
  * @brief Returns the Channel State of the NetDevice NetDevice_id. If the NetDevice is not found, the function returns
  * the static constant state4="NOT FOUND"
  */
  std::string GetChannelState (uint32_t NetDevice_id); //added by Laura

  /**
  * @brief Returns the Channel State of all NetDevices associated to this node. If none is found, vector will be empty
  */
  std::vector<std::string> GetChannelState (); // added by Laura

  /**
  * @brief Start the Channel Load Monitoring
  */
  void StartChannelLoadMonitoring (); // added by J. Haerri (EURECOM)

  /**
  * @brief Schedule the interruption of the Channel Load Monitoring
  */
  void StopChannelLoadMonitoring (); // added by J. Haerri (EURECOM)
...
private:
  ...
  bool m_isMobileNode; // Added by Ramon Bauza first, then by Andrea

  std::vector<Ptr<ChannelLoadMonitorMngr> > m_ChannelLoadMonitorMngr; //Added by Laura first, then by Andrea

  bool m_swState; //Added by Laura
};
