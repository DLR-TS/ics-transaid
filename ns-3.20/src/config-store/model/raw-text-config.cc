  ...
  while (m_is->good ())
    {
      ...
      if (type == "default")
        {
          if(!Config::SetDefaultFailSafe(name, StringValue (value)))
          {
            std::cerr<<"Could not set default value for " << name <<std::endl;
          }
        }
      *m_is >> type >> name >> value;
    }
      ...
      if (type == "global")
        {
          ...
          {
            std::cerr<<"Non-existant global value " << name <<std::endl;
          }
        }
      *m_is >> type >> name >> value;
    }
