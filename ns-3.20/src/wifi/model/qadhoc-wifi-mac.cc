...
// aggiunto per evitare errori di compilazione
void
QadhocWifiMac::SetRifs (Time rifs)
{
...
Time QadhocWifiMac::GetRifs(void) const
{
...
QadhocWifiMac::Enqueue (Ptr<const Packet> packet, Mac48Address to)
{
  ...
  // wifiremotestation non è più una classe, ma una struct. Sposto a wifiremotestationmanager
...
QadhocWifiMac::FinishConfigureStandard (enum WifiPhyStandard standard)
{
  switch (standard)
    ...
    case WIFI_PHY_STANDARD_80211p_CCH:
    	// cambio da ConfigureCCHDcf a ConfigureDcf
...
void
QadhocWifiMac::SetQueuesForSwitchingMngr (Ptr<SwitchingManager> switchingManager)  
{
  switchingManager->SetMacQueue(m_queues[AC_VO]);
  switchingManager->SetMacQueue(m_queues[AC_VI]);
  switchingManager->SetMacQueue(m_queues[AC_BE]);
  switchingManager->SetMacQueue(m_queues[AC_BK]);
}
} //namespace ns3
