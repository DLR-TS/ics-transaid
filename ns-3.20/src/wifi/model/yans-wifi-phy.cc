...
{
  if (Simulator::Now () == Seconds (0) || channelNumberInit)
    {
      // this is not channel switch, this is initialization
      NS_LOG_DEBUG ("start at channel " << nch);
      m_channelNumber = nch;
      channelNumberInit=false;
      return;
    }
...
YansWifiPhy::SendPacket (Ptr<const Packet> packet, WifiMode txMode, WifiPreamble preamble, WifiTxVector txVector)
{
  ...
  //Edit By Federico Caselli and Andrea Cavallari
  m_lastTxPower = txVector.GetTxPowerLevel();
  //end edit
...
//void YansWifiPhy::SetChannelNumberInit(uint16_t nch)
//{
//  NS_LOG_DEBUG("start at channel " << nch);
//  m_channelNumber = nch;
//}
