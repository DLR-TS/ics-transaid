      ...
      for (int i = 0; i < nbConnection; i++)
        {
          ...
          // modificato: aggiunto il cast all'interno di floor per risolvere un errore di ambiguita'
          // che prima dell'integrazione dei moduli itetris non c'era
          symbolsRequired[i] = (uint32_t) std::floor ((double)(symbolsRequired[i] * delta));
