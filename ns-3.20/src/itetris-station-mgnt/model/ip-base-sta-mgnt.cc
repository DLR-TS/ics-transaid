...
//ATTENTION!!! Don't know if this is correct

Ipv6Address* 
IpBaseStaMgnt::GetIpv6MulticastAddress (void) const
{
  Ipv6Address* address = NULL;
  Ptr<Ipv6> ipStack = m_node->GetObject <Ipv6> (); 
  if (ipStack != NULL && ipStack->GetNInterfaces () > 1)
    {
      Ipv6InterfaceAddress ipInterface = ipStack->GetAddress (1,0);
      address = new Ipv6Address (ipInterface.GetAddress ());
      NS_LOG_INFO ("IP broadcast address  = " << *address);
    }
  return address;
}
