...
namespace ns3 {
    ...
    PropagationLossModel::CalcRxPower(double txPowerDbm,
            Ptr<MobilityModel> a,
            Ptr<MobilityModel> b,
            int CarsNumBlock) const {
        double self = DoCalcRxPower(txPowerDbm, a, b, CarsNumBlock);
        if (m_next != 0) {
            self = m_next->CalcRxPower(self, a, b, CarsNumBlock);
        }
        return self;
    }*/
    ...
    NS_OBJECT_ENSURE_REGISTERED (WinnerB1PropagationLossModel);

    TypeId
    WinnerB1PropagationLossModel::GetTypeId (void)
    {
      static TypeId tid = TypeId ("ns3::WinnerB1PropagationLossModel")
        .SetParent<PropagationLossModel> ()
        .SetGroupName ("Propagation")
        .AddConstructor<WinnerB1PropagationLossModel> ()
        .AddAttribute ("Frequency",
                       "The carrier frequency (in Hz) at which propagation occurs  (default is 5.15 GHz).",
                       DoubleValue (5.9e9),
                       MakeDoubleAccessor (&WinnerB1PropagationLossModel::SetFrequency,
                                           &WinnerB1PropagationLossModel::GetFrequency),
                       MakeDoubleChecker<double> ())
        .AddAttribute ("SystemLoss", "The system loss",
                       DoubleValue (1.0),
                       MakeDoubleAccessor (&WinnerB1PropagationLossModel::m_systemLoss),
                       MakeDoubleChecker<double> ())
        .AddAttribute ("MinLoss",
                       "The minimum value (dB) of the total loss, used at short ranges. Note: ",
                       DoubleValue (0.0),
                       MakeDoubleAccessor (&WinnerB1PropagationLossModel::SetMinLoss,
                                           &WinnerB1PropagationLossModel::GetMinLoss),
                       MakeDoubleChecker<double> ())
        .AddAttribute ("hbs",
    				  "Effective Height of BS Antenna",
    				  DoubleValue (1.5),
    				  MakeDoubleAccessor (&WinnerB1PropagationLossModel::SetHbs,
    									  &WinnerB1PropagationLossModel::GetHbs),
    				  MakeDoubleChecker<double> ())
        .AddAttribute ("hms",
    				  "Effective Height of MS Antenna",
    				  DoubleValue (1.5),
    				  MakeDoubleAccessor (&WinnerB1PropagationLossModel::SetHms,
    									  &WinnerB1PropagationLossModel::GetHms),
    				  MakeDoubleChecker<double> ())
      ;
      return tid;
    }

    WinnerB1PropagationLossModel::WinnerB1PropagationLossModel ()
    {
    }
    void
    WinnerB1PropagationLossModel::SetSystemLoss (double systemLoss)
    {
      m_systemLoss = systemLoss;
    }
    double
    WinnerB1PropagationLossModel::GetSystemLoss (void) const
    {
      return m_systemLoss;
    }
    void
    WinnerB1PropagationLossModel::SetMinLoss (double minLoss)
    {
      m_minLoss = minLoss;
    }
    double
    WinnerB1PropagationLossModel::GetMinLoss (void) const
    {
      return m_minLoss;
    }

    void
    WinnerB1PropagationLossModel::SetHbs (double hbs)
    {
      m_hbs = hbs;
    }
    double
    WinnerB1PropagationLossModel::GetHbs (void) const
    {
      return m_hbs;
    }

    void
    WinnerB1PropagationLossModel::SetHms (double hms)
    {
      m_hms = hms;
    }
    double
    WinnerB1PropagationLossModel::GetHms (void) const
    {
      return m_hms;
    }

    void
    WinnerB1PropagationLossModel::SetFrequency (double frequency)
    {
    ...
    double
    WinnerB1PropagationLossModel::GetFrequency (void) const
    {
      return m_frequency;
    }

    double
    WinnerB1PropagationLossModel::DbmToW (double dbm) const
    {
    ...
    double
    WinnerB1PropagationLossModel::DbmFromW (double w) const
    {
    ...
    double
    WinnerB1PropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                              Ptr<MobilityModel> a,
                                              Ptr<MobilityModel> b) const
    {
      ...
      double dBP = 4.0 * m_hbs * m_hms * fGHz * 1.0e9 / C;
      ...
      double PLfree = 20 * log10(distance) + 46.4 + 20 * log10(fGHz / 5); // free space pathloss

      double PL;
          if (distance < dBP)
              PL = 22.7 * log10(distance) + 27.0 + 20.0 * log10(fGHz);
          else
              PL = 40.0 * log10(distance) + 7.56 - 17.3 * log10(m_hbs)
                      - 17.3 * log10(m_hms) + 2.7 * log10(fGHz);

          double PL_db = std::max(PLfree, PL);

          return txPowerDbm - PL_db;

          NS_LOG_DEBUG ("distance=" << distance<< "m, loss=" << PL_db <<"dB");
    ...
    int64_t
    WinnerB1PropagationLossModel::DoAssignStreams (int64_t stream)
    {
      return 0;
    }
    // ------------------------------------------------------------------------- //
    ...
    // ------------------------------------------------------------------------- //
    /*WINNER B1+ - freeway LOS (copied and edited from LogDistancePropatationModel)
     IT adds pathloss and shadowing
     */
    
    NS_OBJECT_ENSURE_REGISTERED(WINNERB1plusFreeWayPropagationLossModel);

    TypeId
    WINNERB1plusFreeWayPropagationLossModel::GetTypeId(void) {
        static TypeId tid = TypeId("ns3::WINNERB1plusFreeWayPropagationLossModel")
                .SetParent<PropagationLossModel> ()
                .SetGroupName("Propagation")
                .AddConstructor<WINNERB1plusFreeWayPropagationLossModel> ()
                .AddAttribute("StdShadowFading",
                "The std of the ShadowFading",
                DoubleValue(3.0),
                MakeDoubleAccessor(&WINNERB1plusFreeWayPropagationLossModel::SetStdShadowFading,
                &WINNERB1plusFreeWayPropagationLossModel::GetStdShadowFading),
                MakeDoubleChecker<double> ())
                .AddAttribute("Frequency",
                "The carrier frequency (in Hz) at which propagation occurs  (default is 5.9 GHz).",
                DoubleValue(5.9e9),
                MakeDoubleAccessor(&WINNERB1plusFreeWayPropagationLossModel::SetFrequency,
                &WINNERB1plusFreeWayPropagationLossModel::GetFrequency),
                MakeDoubleChecker<double> ())
                ;
        return tid;
    ...
    void
    WINNERB1plusFreeWayPropagationLossModel::SetFrequency(double frequency) {
        m_frequency = frequency;
        static const double C = 299792458.0; // speed of light in vacuum
        m_lambda = C / frequency;
    }
    ...
    double
    WINNERB1plusFreeWayPropagationLossModel::GetFrequency(void) const {
        return m_frequency;
    }

    WINNERB1plusFreeWayPropagationLossModel::WINNERB1plusFreeWayPropagationLossModel() {
    }

    void
    WINNERB1plusFreeWayPropagationLossModel::SetStdShadowFading(double sigma) {
        m_sigma = sigma;
    }

    double
    WINNERB1plusFreeWayPropagationLossModel::GetStdShadowFading(void) const {
        return m_sigma;
    }

    double
    WINNERB1plusFreeWayPropagationLossModel::DoCalcRxPower(double txPowerDbm,
            Ptr<MobilityModel> a,
            Ptr<MobilityModel> b) const {
        
        //d’BP = 4 h’BS * h’MS * fc/c
        double dp_BP = 4 * (1.5) * (1.5) / m_lambda;
        
        double distance = a->GetDistanceFrom(b);
        
        if (distance <= 3.0) { //3GPP 36.885 "Pathloss at 3 m is used if the distance is less than 3 m."
            distance = 3.0;
        }
        
        double pathlossDb;
        if (distance <= dp_BP) {
            //PL = 22.7*log10(d) + 27.0 + 20.0*log10(fc)
            pathlossDb = 22.7 * std::log10(distance) + 27.0 + 20.0 * std::log10(m_frequency/1e9);
        }else {
            //PL = 40.0*log10(d) + 7.56 - 17.3*log10(h'BS) - 17.3*log10(h'MS) + 2.7*log10(fc) 
            pathlossDb = 40.0 * std::log10(distance) + 7.56 - 17.3 * std::log10(1.5) - 17.3 * std::log10(1.5) + 2.7 * std::log10(m_frequency/1e9);
        }
        
        m_NormalRandomVariable->SetAttribute ("Mean", DoubleValue (0.0));
        m_NormalRandomVariable->SetAttribute ("Variance", DoubleValue (m_sigma*m_sigma));
        double RV = m_NormalRandomVariable->GetValue();
        
        /*std::cout << "WINNER: dp_BP= " << dp_BP << ", distance= " << distance << ", txPowerDbm= " << txPowerDbm << ", freq= " << m_frequency <<
                     ", pathlossDb= " << pathlossDb << ", m_sigma= " << m_sigma << ", StdShadowing= " << RV << 
                     ", Return power= " << (txPowerDbm - pathlossDb + RV) << std::endl;*/
        
        return txPowerDbm - pathlossDb + RV;
    }

    int64_t
    WINNERB1plusFreeWayPropagationLossModel::DoAssignStreams(int64_t stream) {
        return 0;
    }

    // ------------------------------------------------------------------------- //
    
    //I copied Friis and changed Friis->Baldo
    //Main change is in the DoCalcRxPower function
    NS_OBJECT_ENSURE_REGISTERED(BaldoPropagationLossModel);

    TypeId
    BaldoPropagationLossModel::GetTypeId(void) {
        static TypeId tid = TypeId("ns3::BaldoPropagationLossModel")
                .SetParent<PropagationLossModel> ()
                .SetGroupName("Propagation")
                .AddConstructor<BaldoPropagationLossModel> ()
                .AddAttribute("Frequency",
                "The carrier frequency (in Hz) at which propagation occurs  (default is 58.32 GHz).",
                DoubleValue(58.32e9),
                MakeDoubleAccessor(&BaldoPropagationLossModel::SetFrequency,
                &BaldoPropagationLossModel::GetFrequency),
                MakeDoubleChecker<double> ())
                .AddAttribute("SystemLoss", "The system loss",
                DoubleValue(1.0),
                MakeDoubleAccessor(&BaldoPropagationLossModel::m_systemLoss),
                MakeDoubleChecker<double> ())
                .AddAttribute("MinLoss",
                "The minimum value (dB) of the total loss, used at short ranges. Note: ",
                DoubleValue(0.0),
                MakeDoubleAccessor(&BaldoPropagationLossModel::SetMinLoss,
                &BaldoPropagationLossModel::GetMinLoss),
                MakeDoubleChecker<double> ())
                ;
        return tid;
    }

    BaldoPropagationLossModel::BaldoPropagationLossModel() {
    }

    void
    BaldoPropagationLossModel::SetSystemLoss(double systemLoss) {
        m_systemLoss = systemLoss;
    }

    double
    BaldoPropagationLossModel::GetSystemLoss(void) const {
        return m_systemLoss;
    }

    void
    BaldoPropagationLossModel::SetMinLoss(double minLoss) {
        m_minLoss = minLoss;
    }

    double
    BaldoPropagationLossModel::GetMinLoss(void) const {
        return m_minLoss;
    }

    void
    BaldoPropagationLossModel::SetFrequency(double frequency) {
        m_frequency = frequency;
        static const double C = 299792458.0; // speed of light in vacuum
        m_lambda = C / frequency;
    }
    ...
    double
    BaldoPropagationLossModel::GetFrequency(void) const {
        return m_frequency;
    }

    double
    BaldoPropagationLossModel::DbmToW(double dbm) const {
        double mw = std::pow(10.0, dbm / 10.0);
        return mw / 1000.0;
    }

    double
    BaldoPropagationLossModel::DbmFromW(double w) const {
        double dbm = std::log10(w * 1000.0) * 10.0;
        return dbm;
    }

    double
    BaldoPropagationLossModel::DoCalcRxPower(double txPowerDbm,
            Ptr<MobilityModel> a,
            Ptr<MobilityModel> b) const {
        
         // "Path-Loss Prediction Models for Intervehicle Communication at 60GHz"
         // rx = Gt + Gr + tx - L
         // L = A*10*log10(D) + C + 15*D/1000 
         //   with D= distance between the TX and RX
         // --------------------------------------------------
         //      |       A               |       C[dB]        |
         // --------------------------------------------------|
         //      |50%    |10%    |90%    |50%    |10%    |90% |
         // -------------------------------------------------- 
         // LOS  |1.77   |1.84   |2.10   |70.0   |65.9   |75.1|
         // 1veh |1.71   |2.1    |1.22   |78.6   |67.3   |94.6|
         // 2veh |0.635  |1.11   |0.453  |115    |97.8   |126 |
         // 3veh |0.362  |0.729  |0.240  |126    |111    |135 |
         // --------------------------------------------------
         // rx: rx power (dB)
         // tx: tx power (dB)
         // D: distance (m)
         // LS: system loss (unit-less)
         // lambda: wavelength (m)
        ...
        double lossDb = A * 10 * std::log10(distance) + C + 15 * distance / 1000;
        std::cout << "NumCarsBlocking: " << CarsNumBlock << ", distance (m): " << distance << ", loss (dB): " << lossDb << ", rx_power(dB): " << txPowerDbm - std::max(lossDb, m_minLoss) << std::endl;
        NS_LOG_DEBUG("distance=" << distance << "m, CarsNumBlock=" << CarsNumBlock << ", loss=" << lossDb << "dB");
        return txPowerDbm - std::max(lossDb, m_minLoss);
    }

    int64_t
    BaldoPropagationLossModel::DoAssignStreams(int64_t stream) {
        return 0;
    }
    ...
    FriisPropagationLossModel::DoCalcRxPower(double txPowerDbm,
        ...
        double numerator = m_lambda * m_lambda;
        double denominator = 16 * M_PI * M_PI * distance * distance * m_systemLoss;
        double lossDb = -10 * std::log10(numerator / denominator);
        NS_LOG_DEBUG("distance=" << distance << "m, loss=" << lossDb << "dB");
        return txPowerDbm - std::max(lossDb, m_minLoss);
    }
