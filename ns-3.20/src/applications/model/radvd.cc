...
void
Radvd::ConfigureNode(Ipv6Address address, Ptr<Packet> p)
{
	...
	for(uint32_t i=0;i<netDeviceNumber;i++)
	{
		if(DynamicCast<UMTSNetDevice>(m_node->GetDevice(i)))
		{
			/*if(!=m_rrcicmpenabled)
		  	  {
  	  	  	  	  SetAppIndex(m_node->GetNApplications());
  	  	  	  	  AppIndexTag app_index_tag;
	  	  	  	  app_index_tag.Set(m_app_index);
				  p->AddPacketTag(app_index_tag);*/
...
void Radvd::SetAppIndex (uint32_t app_index)
{
...
void Radvd::Send (Ptr<RadvdInterface> config, Ipv6Address dst, bool reschedule)
{
	...
	//New part for RRC
	m_app_index_tag.Set(m_app_index);
	p->AddPacketTag(m_app_index_tag);
