...
public:
  ...
  */
  ...
  /**
   * \brief Add App index for the radvd application (for RRC use).
   * \param app_index the value of the index.
   */
  void SetAppIndex (uint32_t app_index);

  /**
   * \brief Configure the ICMP application for the node (for NS and RS).
   * \param address the destination address
   */
  void ConfigureNode(Ipv6Address address, Ptr<Packet> p);
...
private:
  ...
  /**
   * \brief Indicate whether we have configured the rrc flow for radvd or not.
   */
  bool m_rrcconfigured;

  /**
   * \brief The flow id for ICMPv6 in RRC Layer.
   */
