...
class GccTraits(CompilerTraits):
	...
	#jin: delete werror part; ignore warnings
		self.warnings_flags = [['-Wall'], ['-Wextra']]
		#self.warnings_flags = [['-Wall'], ['-Werror'], ['-Wextra']]


	#jin: delete wall part as well; comment all all following part
	def get_warnings_flags(self, level):
		warnings = []
...
class MsvcTraits(CompilerTraits):
	def __init__(self):
		...
		# cumulative list of warnings per level
		#jin : delete regarding parts for werrors
		#self.warnings_flags = [['/W2'], ['/WX'], ['/Wall']]
		self.warnings_flags = [['/WX']]
